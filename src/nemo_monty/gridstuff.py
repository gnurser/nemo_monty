...
def grid_from_fext(fext):
    fextgrid =  {'t':{'T','I','P','SIGI'},'u':{'U','U2'},'v':{'V','V2'},'w':{'W','W2','LSPV'},'f':{'PSI'}}
...
class GridStuff():
    def __init__(self,grids):
        ...
        self.bdsgrid_disp = {'t':(0,0),'u':(0,1),'v':(1,0),'f':(1,1),'w':(0,0),'vw':(1,0)}
...
class RealGridStuff(GridStuff):
    ...
    def fitslice(Nddict,name,slice):
        ...
        if ndslice > ndim_real:
            ...
            slice = (ndim-ndim_real)*(0,) + int(ndim_real>ndslice)*(Ellipsis,) + slice
            return Nd[slice]
                ...
                if 'bnds' in vble:
                    ...
                    for cdfvble in cdfvbles:
                        if cdfvble in avail_vars:
                            ...
                            meshdict[vble][...,1] = bds[dj:ny+dj,di+1:nx+1+di]
                            meshdict[vble][...,2] = bds[dj+1:ny+1+dj,di+1:nx+1+di]
                            meshdict[vble][...,3] = bds[dj+1:ny+1+dj,di:nx+di]
                            if verbose:
                    ...
                    for cdfvble in cdfvbles:
                        if cdfvble in avail_vars:
                            ...
                            if ndim_real == 1:
                                ...
                                meshdict[vble] = self.fitslice(f.variables,cdfvble,hslice)
                            print (vble, ' has cdf variable ',cdfvble, ' dtype ',
                              meshdict[vble].dtype, ' and shape ',meshdict[vble].shape, file=self.wfile)
                            if verbose:
                                print('Created mesh[ %s ].%s' % (grid, vble))
                            else:
                                print ( 'Created mesh[ %s ].%s' % (grid, vble), file=self.wfile)
    ...
    def find_meshes(self,meshkeys,meshdir,hslice,wide_hslice=None,meshfile=None, meshbnds=False,verbose=False):
        ...
        print(meshkeys)
        if meshbnds:
            if 'glam' in meshkeys: meshkeys.append('glambnds')
            if 'gphi' in meshkeys: meshkeys.append('gphibnds')
        Mesh = namedtuple('Mesh', meshkeys)
        meshes = {}
        self.hslice = hslice

...
def simple_glob(rootdir=".", suffix="", diagfile=None):
    ...
    paths = [
        ...
        and os.path.isfile(pjoin(rootdir, f))
        and not splitext(f)[0][-4:].isdigit()
    ]
    return paths
...
def get_dirs(basedir):
    ...
    return [name for name in os.listdir(basedir) if isdir(pjoin(basedir, name))]
...
class Find_Run(object):
    ...
    def __init__(self):
        ...
        rootdirs = {
            "centos": ["/hpcdata/scratch/omfman/ALTIX1", "/hpcdata/scratch2"],
            "macOS": [
                "/Volumes/Achernar/Data/NEMO/ORCA12",
...
class Run(object):
    ...
    def __init__(
        self, runname, refresh=False, writecache=True, excludes=[], subdir=None
    ):
        ...
        if cache:
            ...
            self.valfiles = recursive_glob(
                rootdir=rootdir, suffix="nc", excludes=excludes
            )
            print("found files")
            if writecache:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-y", dest="years", help="Year(s)", type=int, nargs="*", default=None
    )
    parser.add_argument("-m", dest="month", help="Month", type=int, default=None)
    parser.add_argument(
        "--pass", dest="passno", type=int, help="pass number", default=None
    )
    parser.add_argument(
        "--day5", dest="day5", help="last 5day of 5-day period", type=int, default=None
    )

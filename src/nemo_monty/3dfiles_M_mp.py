...
class Montgomery(Rho):
    ...
    def calc(self, sshd, Td, Sd):
        ...
        self.z_median_km = ma.median(self.z_s) * 1.0e-3
        self.sig_s, self.sig_s_zmed = [
...
class K_below_s(object):
    ...
    def describe(self, montg_instance=None):
        self.data.long_name = (
            "Fortran k index just above surface r_B = %5.2f" % montg_instance.d0
        )
...
class SigmaMedian_s(Sigma0_s):
    ...
    def calc(self, montg_instance):
        ...
        self.data.long_name = "Potential density ref to %5.2f km on r_B = %5.2f" % (
            montg_instance.z_median_km,
            montg_instance.d0,
        )
        self.setlims()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--density",
        dest="density",
        help="layer density for layer output",
        type=float,
        default=None,
    )
    parser.add_argument(
    ...
    for fext in fexts:
        if use_infile:
            ...
            pathname = model_run(
                passno=passno0, years=year0, month=months[0], fext=fext, day5=days[0]
            )
    ...
    def do_date(p, y, month, day, first):
        if first:
            ...
            for fext in fexts:
                pathname = model_run(
                    passno=p, years=y, month=month, fext=fext, day5=day
                )
    ...
    def do_infile(infile):
        ...
        print(
            f"time  before writing to output file & after"
